...
def fun1() : # fun1함수는 chk.get() 함수로 체크버튼에 설정된 값을 가져와서 메시지를 출력한다.
    ...
    if chk6.get() == 1 :
        ...
        menu6 = ""
    messagebox.showinfo('아이스크림 메뉴', menu1 + " " + menu2 + " " + menu3 + " " + menu4 + " " + menu5 + " " + menu6)
...
def fun2() : # fun2 함수는 var 변수 값으로 맨 아래쪽 레이블의 텍스트를 변경한다.
    if rVar.get() ==1 :
        ...
        lang = "하프갤런 26500원"
    messagebox.showinfo('아이스크림 사이즈', lang)
...
lal2 = Label(w, text="아이스크림 사이즈", fg="red")
##Radiobutton 위젯사용하기
...
btnOk1 = Button(w, text="아이스크림 사이즈", fg="#ff0000" , command=fun2)  #=16진수,#ff=4비트, #ffffff=흰색, #ff0000=빨강, #00ff00=그린, #ffff00=노랑
